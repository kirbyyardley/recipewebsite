...
const SheetWithDepthStack = ({ children }: any) => {
   ...
   return (
      <SheetWithDepthStackContext.Provider value={contextValue}>
         <SheetStack.Root componentId={sheetWithDepthStackId}>{children}</SheetStack.Root>
...
const SheetWithDepthStackScenery = ({ children, ref, ...restProps }: any) => {
   const { iOSStandalone, nativePageScrollReplaced } = useContext(SheetWithDepthStackContext);
...
const SheetWithDepthStackBackground = () => {
   const { nativePageScrollReplaced, stackBackgroundRef } = useContext(SheetWithDepthStackContext);
...
const SheetWithDepthStackFirstSheetBackdrop = () => {
   ...
   return (
      ...
      // here, but and an independant animation to have a
      // different, smoother easing.
      <div className="SheetWithDepth-stackFirstSheetBackdrop" ref={stackFirstSheetBackdropRef} />
   );
};
...
const SheetWithDepthRoot = ({ children, ...restProps }: SheetWithDepthRootProps) => {
   return (
      <Sheet.Root license="commercial" forComponent={sheetWithDepthStackId} {...restProps}>
         {children}
      </Sheet.Root>
   );
};
...
interface SheetWithDepthViewProps extends SheetViewProps, React.HTMLAttributes<HTMLDivElement> {
   children: React.ReactNode;
}

const SheetWithDepthView = ({ children, onTravelStatusChange }: SheetWithDepthViewProps) => {
   const { stackingCount, setStackingCount, nativePageScrollReplaced, stackBackgroundRef, stackFirstSheetBackdropRef } =
   ...
   const travelStatusChangeHandler = useCallback<Exclude<SheetViewProps["onTravelStatusChange"], undefined>>(
      (newTravelStatus) => {
         // * Run passed handler
   ...
   const travelHandler = useMemo(() => {
      if (indexInStack === 1 && travelStatus !== "entering" && travelStatus !== "exiting") {
         return ({ progress }: any) => {
            ...
            stackFirstSheetBackdropRef.current.style.setProperty("opacity", (progress * 0.33) as unknown as string);
         };
      } else return undefined;
   }, [indexInStack, travelStatus, nativePageScrollReplaced, stackFirstSheetBackdropRef, setDimmingOverlayOpacity]);
      ...
      <Sheet.Portal>
         ...
         >
            ...
            {stackingCount > 0 && indexInStack !== 1 && (
               <Sheet.Backdrop className="SheetWithDepth-backdrop" travelAnimation={{ opacity: [0, 0.33] }} />
            )}
            <Sheet.Content
